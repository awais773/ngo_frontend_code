    ...
    <section className="py-12 lg:py-16 bg-white dark:bg-dark" data-aos="fade-up" data-aos-duration="1200">
      <div className="container mx-auto lg:max-w-(--breakpoint-xl) px-4">
        <div className="flex justify-between items-end mb-12">
          <div>
            <BrandEyebrow>Latest News</BrandEyebrow>
            <h2 className="text-3xl font-semibold text-darkprimary dark:text-white">From our blog</h2>
          </div>
          <Link href="/blog" className="text-secondary font-semibold hover:text-darkprimary dark:hover:text-accent transition-colors hidden sm:block">
            View all →
          </Link>
        </div>
          ...
          {blogs.slice(0, 3).map((blog, index) => (
            ...
            >
              <div className="relative h-48 rounded-xl overflow-hidden mb-4 border border-beige-dark dark:border-dark_border">
                ...
                />
                {blog.category && (
                  <span className="absolute top-3 left-3 bg-secondary text-white text-xs px-2 py-1 rounded font-medium">{blog.category}</span>
                )}
              </div>
              <p className="text-xs text-dustGray mb-2">
                {blog.published_at ? format(new Date(blog.published_at), "MMM dd, yyyy") : ""}
              </p>
              <h3 className="font-bold text-lg text-midnight_text dark:text-white group-hover:text-secondary dark:group-hover:text-secondary line-clamp-2">{blog.title}</h3>
